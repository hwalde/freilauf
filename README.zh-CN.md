...
- **四个 CLI，一个入口 —— 以及这里从没听说过的第五个。** hub 可以驱动哪些编码
  智能体、每个智能体可以使用哪些模型供应商，都是配置项，而不是代码改动；而以插件
  包形式发布的编码智能体或供应商，根本不需要改动本仓库。

## 功能一览

- **首次访问时的五步欢迎向导**：这台机器上装了什么、第一个编码智能体、第一个模型
  供应商，以及由哪个模型来回答 hub 自己的小问题。可以跳过，也可以永久关闭。
- **编码智能体与模型供应商都是插件** —— claude、opencode、hermes、cursor 以及
  OpenRouter/DeepSeek/OpenCode Zen 随附，**第三方也可以把一个插件包放到机器上**
  （`CCHUB_PLUGIN_DIR`），启动时自动加入。插件可以自带 API 密钥处理方式、自带预算
  闸门阈值，还可以带一份启动声明，让 tmux 启动器去运行一个这里从没见过的 CLI
  （[docs/plugins.md](docs/plugins.md)）。
- **一个插件页面**（设置 → 插件）负责安装、配置、启用与移除插件，并展示启动扫描在
  这台机器上发现了什么。供应商的密钥可以放在你选定的环境变量里 —— 也可以直接填在
  这个页面上，适合那些不方便再加一个环境变量的机器。
- **hub 自己的小问题** —— 给运行起名、判断某条日志是不是真的故障、读取报告、推荐
  worktree 附加文件 —— 每一项都交给你为它选定的模型来源。其中也包括**你已经在付费
  的订阅上的编码智能体**，这正是让 hub 在没有任何 API 密钥时仍然可用的办法。
...
- **订阅用量** —— Claude 的 5 小时 / 7 天窗口、Cursor 当前周期的花费、OpenRouter
  ...
  Claude 运行，按模型划分的周窗口只约束使用该模型的运行，Cursor 运行只受其周期用量约束，
  DeepSeek 运行只受其自身
  余额约束。每个闸门都是**可选的**，各有自己的阈值（设置 → 预算闸门），被推迟的
  运行也可以从其详情页**强制启动**。同一个侧边栏还会显示本机所有 tmux 会话共占用多少内存，每八分钟
  重新测量一次：会话是有意在其智能体之后继续存活的，这笔账会悄悄累积。一个可配置的 **tmux 清理代理**
  （设置 → tmux 清理，与运行表单相同的 代理+供应商+模型 选择）会在内存超过阈值时结束最旧的闲置会话，
  直到达到你选择的目标内存 —— 也可按需启动：通过侧边栏 tmux 区块的按钮和会话页面上的提示框。
...
**进入界面后的第一件事**：**欢迎向导** —— 首次访问 `/` 就会到那里。它会带你过一遍
这台机器上装了什么、你的第一个编码智能体、第一个模型供应商，以及回答 hub 自己那些
小问题的模型；勾选"不再显示"即可让它退场。这些事情之后也都能在 **设置 → 插件**
里做，全新安装时每个页面都会有横幅指向那里。可选的种子文件
`~/.config/cc-hub/coding-agents.json` 会在首次启动时预填编码智能体，这正是脚本化
安装可复现的关键。
...
**尽管 fork、修改、拆掉不需要的部分。** 有意为你留出的接缝：**编码智能体与模型
供应商插件 —— 也可以是完全活在本仓库之外的插件包**、平台提示词后缀、按仓库的
提示词、`~/agents/zusaetze/` 中可选启用的额外技能、hub 自己那些问题背后的模型来源，
以及无代码流程。完整表格见
[SETUP_WITH_AGENT.md](SETUP_WITH_AGENT.md)，完整的插件契约见
[docs/plugins.md](docs/plugins.md)。
