...
- **需要的话，给智能体加一道围墙。** 一次运行可以在容器里工作：用自己的克隆而不是
  ...
  关闭**；只有你想要它时才需要 Docker。有两条关于验证到什么程度的事实，应当写在
  概要里，而不是只写在细则中。在 **rootless** 守护进程上——也就是我们推荐的部署
  形态——强制生效的白名单现在可以工作了，因为那里的代理本身就是一个跑在该次运行
  自有网络上的容器；这一点是对着真实客户端测出来的（`git`、`npm`、`curl`，以及
  运行期间改规则），但**还没有任何编码智能体在它后面工作过**，所以请先从只审计
  模式开始，并预期会学到一些关于你自己白名单的东西。还有：**已经有一个编码智能体
