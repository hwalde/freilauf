...
- **一个窗口。** 正在运行什么、花了多少、产出了什么、哪里需要你——总览、浏览器里的
  实时终端（xterm.js，默认只读；在其中选中的文本会自动进入剪贴板）、直接链接到运行的通知。侧边栏显示你的订阅窗口、
  提供商余额，以及这台机器上每个 tmux 会话占用的内存；可配置的清理智能体会在占用
  过大时结束最老的空闲会话。**你自己项目的数字也在那里**——按类型统计的未决问题、
  失败的测试，凡是它统计的：项目把数字推送过来（`fl-panel`，一次 HTTP 调用），
  Freilauf 连同测量时间一起显示，却从不知道它们的含义
  （[docs/panels.md](docs/panels.md)）。
- **需要的话，给智能体加一道围墙。** 一次运行可以在容器里工作：用自己的克隆而不是
  你的检出、自己的 home，以及一个唯一出口是代理的网络——不在白名单上的主机会收到一个
  可读的 403。这道边界在运行开始**之前**就配置好：在中枢、按仓库、按智能体、按单次
  运行，而下层只能收紧上层锁定的东西，绝不能放宽。当它挡住了智能体真正需要的东西时，
  你会看到那个主机，一次点击就能为这次运行或这个仓库放行，无需重启。可选，且**默认
  关闭**；只有你想要它时才需要 Docker。验证到什么程度，应当写在概要里，而不是只写在
  细则中：**四个编码智能体现在都已经在容器里做过真实的工作**——写文件、提交、回报，
  并合并进基础分支——而且**其中一个（opencode）已经在强制生效的白名单后面工作过**：
  白名单之外的主机被拒绝，这次拒绝变成了一个事件，而放行在该智能体的下一次尝试时
  就已生效，无需重启。即便如此也请先从只审计模式开始，并预期会学到一些关于你自己
  白名单的东西——它正是为此而存在（[docs/sandbox.md](docs/sandbox.md)——那份文档
  讲它**不做**什么，和讲它做什么一样详细）。
...
- **可选地，智能体本身跑在容器里**——自己的克隆、自己的 home、默认拒绝的出站白名单、
  丢弃全部 capabilities、只读根文件系统和 cgroup 限制。默认关闭。它是**智能体**与
  宿主机之间的一道墙，由中枢建起，而中枢站在这道墙的宿主机一侧：它不会缩小一个被
  攻陷的中枢能做的事，也不检查任何流量内容——所以一个被放行的主机就是一条出路。请读
  [docs/sandbox.md](docs/sandbox.md)，其中「What this sandbox does not do」一节
  才是关键。
