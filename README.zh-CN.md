...
- **每次运行一个工作区。** 每次运行都有自己的 git 工作树和 tmux 会话。智能体之间
  互不干扰——也不会干扰你；事后你可以接入任何一个会话，读完整个屏幕。已结束的会话
  可以被唤醒——同一个智能体延续同一段对话，或由拿到全部记录的新智能体接手——次数不限。
...
- **需要时可启用代码评审。** 与沙箱一样是可选的：可在全局、每个仓库以及每个
  智能体或运行上设置。完成的工作会被推送并等待评审，而不是直接进入基础分支：
...
- **需要的话，给智能体加一道围墙。** 一次运行可以在容器里工作：用自己的克隆而不是
  ...
  你会看到那个主机，一次点击就能为这次运行或这个仓库放行，无需重启。还有一个
  只审计模式：它记录一次运行访问了什么却不加拦截——这是在正式强制之前摸清自己
  白名单的稳妥方式。可选，且**默认关闭**；只有你想要它时才需要 Docker
...
- **可选地，智能体本身跑在容器里**——自己的克隆、自己的 home、默认拒绝的出站白名单、
  ...
  [SANDBOX.md](SANDBOX.md)，其中「What this sandbox does not do」一节才是关键。
