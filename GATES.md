...
- [x] G12: the test suites cannot reach the operator's real skill directories
  EVIDENCE: `FREILAUF_SKILLS_HOME` and `FREILAUF_SKILLS_STATE` are set in `test/sandbox-env.mjs` (both in the hub's environment and in `prepareWatcher`) and at the top of `test/unit.mjs`; the e2e test additionally asserts that every resolved target directory starts with the sandbox home before it installs anything.
