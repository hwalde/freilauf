...
export function runInfo(runId) {
  ...
  return {
    ...
    // Owned by the merge integrator and only read here. An installation whose
    // database does not carry those columns yet reports them empty rather than
    // making every flow blind — `''` is the same answer `outcome` gives while a
    // run is still going.
