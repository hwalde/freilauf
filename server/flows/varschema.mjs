...
// Where a run's work ended up — what the merge integrator writes into
// `runs.merge_status` (server/integrate.mjs). Copied here rather than imported,
// because this module has to stay free of anything the browser cannot run. The
// empty string leads the list on purpose: it is what a run reports while nothing
// has been decided yet, and what every run of an installation without the
// integrator reports forever — so a flow can ask "did this really land on main?"
