...
'',
'- **Running runs, and runs in an open code review** (a reviewer\'s change request goes into that session). Query them first and except them:',
'```bash',
'sqlite3 "{db}" "SELECT tmux_session FROM runs WHERE (status IN (\'running\',\'waiting_help\') OR (review_platform NOT IN (\'none\') AND merge_status IN (\'in_review\',\'changes_requested\',\'approved\'))) AND tmux_session IS NOT NULL;"',
'```',
