#!/usr/bin/env bash
# 04-firewall.sh — ufw rules for Freilauf (REQUIRES SUDO — run once).
...
ufw deny in to any port "$VPN_PORT" proto tcp comment 'Block external Freilauf'
...
Important: the hub itself binds only to 127.0.0.1:${FREILAUF_LOCAL_PORT:-8791} — without
the proxy (freilauf on) the website is NOT reachable from outside, not even via VPN.
EOF
