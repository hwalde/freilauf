...
| `usage(ctx)` | async fn | subscription usage for the overview panel, or `null`. Shapes in `usage.mjs`: `{kind:'claude', …}` / `{kind:'cursor', plan, spent_usd, included_usd, included_estimated?, bonus_usd?, remaining_usd, auto_pct?, api_pct?, auto_models?, pct, cycle_end}`. For cursor, `auto_pct` is what Cursor prints for Auto (`totalPercentUsed`, "included total usage"), **not** `autoPercentUsed`; `api_pct` is `apiPercentUsed`. `spent_usd` is the included-dollar estimate and is not the bar. |
