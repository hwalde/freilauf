...
`)
  ...
  await check('the three levels are forms: Settings → Code review, the repo, the run', async () => {
    ...
    setSetting('review_platform', 'github')
    contains(await (await fetchPath('/settings')).text(), 'Global default: on (GitHub)', 'a platform plugin is named by its label')
    setSetting('review_platform', 'internal')
    ...
    contains(edit, 'href="/settings/review"', 'the repo form links to the global default it overrides')
    contains(await (await fetchPath('/reviews')).text(), 'href="/settings/review"', 'and so does the Reviews page')
