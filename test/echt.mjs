#!/usr/bin/env node
// cc-hub — the REAL integration suite. No stubs, no fakes, no mocked fetch.
//
...
// throwaway CCHUB_DATA_DIR of their own, set before the first import that
// could reach db.mjs.
//
...
const LIBSB = mkdtempSync(join(tmpdir(), 'cc-hub-echt-lib-'))
process.env.CCHUB_DATA_DIR = LIBSB
process.env.CCHUB_PLUGIN_DIR = join(LIBSB, 'plugins')
mkdirSync(process.env.CCHUB_PLUGIN_DIR, { recursive: true })
 ...
 */
const MODELS = {
  'provider:openrouter': process.env.CCHUB_ECHT_OR_MODEL ?? 'deepseek/deepseek-v4-flash',
  'provider:deepseek': process.env.CCHUB_ECHT_DS_MODEL ?? 'deepseek-v4-flash',
  'provider:opencode-zen': process.env.CCHUB_ECHT_ZEN_MODEL ?? 'ling-3.0-flash-fin-free',
  'agent:claude': process.env.CCHUB_ECHT_CLAUDE_MODEL ?? 'haiku',
  'agent:cursor': process.env.CCHUB_ECHT_CURSOR_MODEL ?? 'composer-2.5',
  'agent:opencode': process.env.CCHUB_ECHT_OC_MODEL ?? 'deepseek/deepseek-v4-flash',
  'agent:hermes': process.env.CCHUB_ECHT_HERMES_MODEL ?? 'deepseek/deepseek-v4-flash',
}
...
// ===========================================================================
const sk = neuerSandkasten({ praefix: 'cc-hub-echt-', behalten: BEHALTEN })
...
try {
  ...
  // The stub cc-start stays: an LLM caller is not an agent run, and section F
...
const plugin = {
  id: 'echt-provider',
  label: 'Echt Provider',
  credentials: [{ key: 'api_key', envKeys: ['CCHUB_ECHT_PROVIDER_KEY'], labelKey: 'plugins.cred_api_key', required: true }],
...
`)
  ...
  if (!HAS.deepseek) uebersprungen('the external provider\'s credential is really used', 'DEEPSEEK_API_KEY is not set (the external provider borrows that account)')
    ...
    await pruefe('a RENAMED environment variable is honoured', async () => {
      ...
      // plugin declares CCHUB_ECHT_PROVIDER_KEY — so a working answer here can
  ...
  await pruefe('cc-start --spec really starts the external coding agent in a tmux session', async () => {
    // The hub is restarted with the REAL cc-start and with the agent's binary on
  ...
  if (!MIT_RUNS) {
    ...
    // cc-start; that is exactly what a real run needs.
    const db3 = sk.db
      ...
      await pruefe(`real run: ${f.harness} writes the file, reports done and passes the finish gate`, async () => {
        ...
        const r = await formular('/api/runs', {
          ...
          prompt: `Create the file ${marke} in the current directory with exactly one line: ${f.harness} ran. `
            + `Then run exactly this command: cc-report done "${f.harness} smoke test finished"`,
        })
        const j = await r.json()
